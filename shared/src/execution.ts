...
const boundedSkillCompleted = z.strictObject({ kind: z.literal("bounded_skill_completed"), skillId: SkillId });
const startFrom = z.discriminatedUnion("kind", [advisoryNone, boundedSkillCompleted]);
...
const denied = <Reason extends string, Guidance extends z.core.$ZodType>(
  reason: Reason,
  guidance: Guidance,
) => z.strictObject({
...
const denialArms = [
  denied("workflow_selection_required", z.strictObject({ kind: z.literal("select_workflow"), nextOperation: z.literal("workflow_start") })),
  denied("skill_selection_required", z.strictObject({ kind: z.literal("select_bounded_skill"), nextOperation: z.literal("bounded_skill") })),
  denied("operation_not_allowed_in_mode", z.strictObject({ kind: z.literal("use_advisory_mode"), requiredMode: z.literal("advisory_preparation") })),
  denied("workflow_start_requires_advisory_context", z.strictObject({ kind: z.literal("use_advisory_mode"), requiredMode: z.literal("advisory_preparation") })),
  denied("workflow_start_requires_completed_skill", z.strictObject({ kind: z.literal("complete_bounded_skill"), nextOperation: z.literal("workflow_start") })),
  denied("workflow_already_selected", z.strictObject({ kind: z.literal("continue_selected_workflow"), nextOperation: z.literal("implementation") })),
] as const;
...
export const tryParseWorkflowStartCommitRequest = (value: unknown): ParseOutcome<WorkflowStartCommitRequest> => tryParseWith(WorkflowStartCommitRequest, value, "response");
export const parseWorkflowStartResult = parseWorkflowStartCommitRequest;
export const tryParseWorkflowStartResult = tryParseWorkflowStartCommitRequest;
