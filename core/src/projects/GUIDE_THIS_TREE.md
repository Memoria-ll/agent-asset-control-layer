...
- startup reconciliation の観測不能は既存 Registry entry を保持する。bounded deadline の timeout は degraded startup とし、Registry 自体の read / parse / lock / write failure は listen 前の startup failure とする。
