...
const ensureProjectDirectory = async (projectRoot: string): Promise<AssetResult<string>> => {
...
const projectIdFrom = (suffix: string): AssetResult<ProjectId> => {
  ...
  return { ok: true, value: `project-${suffix}` as ProjectId };
};
  ...
  return {
    ...
    initialize: async (projectRoot) => {
      ...
      return serialized(normalizedRoot, async () => {
        const prepared = await ensureProjectDirectory(normalizedRoot);
        if (!prepared.ok) return prepared;
        const markerPath = join(prepared.value, PROJECT_MARKER_FILE_NAME);
