...
**設計方針の追加更新:** 後続の相談で、Skillはdescriptionを先に渡して本文を必要時に取得すること、取り込み時に内容からWorkflowへ分類すること、Model割り当てを任意にすること、Skillには担当者や工程を制御する仕組みを持たせないこと、作成・エクスポートを汎用Skillで扱うことが示されました。[最新の整理と推奨動作](skill-workflow-model-and-export-design.md)を参照してください。以下の「Skill内の手順の構造化」と「実行に必須のModel確定」を示唆する記述は、この更新で置き換えます。追加設計の実装と検証は、次の実装確認表に記載しています。
