...
| 初回整理       | `aacl_onboarding_organize`に依頼・理由・変更の配列を渡します。分類、Roleへの紐づけ、適用条件を一括保存できます。未確定な資産は無効・未紐づけのまま残せます。     |
...
Workflowの工程はRoleを選びます。Roleのモデル割り当ては`aacl_config_get`で取得した設定の`bindings`に保存します。全Workflow共通のRole割り当てと、特定Workflowの割り当てを区別できます。設定は`aacl_config_update`へ現在の`settingsVersion`を`expectedVersion`として渡して変更します。
...
| 種類          | 保存と実行時の扱い                                                                                                                     |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `required`    | 必須利用です。対象とその条件を検証し、必要な本文を解決します。必須利用の循環は拒否します。                                             |
...
禁止の記述から利用関係を作りません。IDやパスが曖昧な場合も推測で結びません。自然言語の意味の整理は、ユーザーが依頼したAIによる初回整理・編集で行います。`skill.steps`には、Skill同士の順序、条件、入力、出力を保存できます。工程担当・モデル・権限は引き継ぎ、Coreが別のAIを自動起動する意味にはしません。
