...
| `CapabilityOffer` は `capabilityId` と `features` だけ（`core-domain/src/resolution/capabilities.ts:35-43`）。`capabilityAvailable` も catalog と offer の存在だけを検査する（同:396-404） | available と allowed を capability 側の観測値として分離し、Resolver は検証済み観測値を消費する。catalog / permission 判定を Resolver の pipeline へ複製しない |
...
6. Phase B で capability の型検証・依存観測を `core-domain/src/capabilities/` へ移す。domain の `capabilitySnapshot` は required value とし、明示 empty snapshot は 0 offers の値として fixtures に渡す。S1-S6 / S10 / S13 / S15 を capability owner へ移し、S7-S9 / S16-S17 は Resolver integration として残す。この段階の offer は current availability semantics を保ち、available / allowed の分離は Phase C で行う。
