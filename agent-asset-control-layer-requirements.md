# Agent Asset Control Layer — 開発要件 v15

2026年9月9日更新。[総合改善インプット](docs/integrated-improvement-input-2026-09-09.md)、[利用方針更新](docs/mcp-first-onboarding-and-asset-relations.md)、[Skill・Workflow・モデル・出力の設計](docs/skill-workflow-model-and-export-design.md)を反映しています。
...
複数工程の進行                  → Workflow
役割固有の責務                  → Role
作業目的・変更方針              → Task Type / Workflow Context
...
### 9. Roleは共通責務、作業目的はWorkflow / Task Typeで表現する

Roleは「その実行で何者として振る舞うか」、Modelは「どのモデルが実行するか」を表す。Role自体は、そのRoleに共通する責務を定義する。

同じRoleであっても、実行する作業の目的によって必要な判断基準や制約は異なる。例えばImplementerでも、Issue開発では要求範囲を越えない最小変更を重視し、リファクタリングでは保守性・責務分離・構造最適化を優先してより広い変更を許容しうる。

この差を `minimal-change-implementer` / `refactoring-implementer` のようなRole分割で表現すると、作業種別ごとにRoleが増殖し、RoleとTask Typeの責務が混ざる。

そのため、**Roleは共通責務として再利用し、作業目的によるAsset差はWorkflow / Stage / Task Type / Role等の組み合わせで解決する**ことを基本とする。

```text
Role
  → Implementerとして共通する責務

Task Type / Workflow
  → 今回の作業目的・変更方針

Composite Scope
  → 今回だけ必要なSkill / Rule / Knowledge / Policy
```
...
Specifier / Reviewer等についても同様とし、作業目的の差だけを理由にRoleを増殖させない。
...
Roleは作業種別ごとの差ではなく、そのRoleに共通する責務を持つ。Issue開発用Implementer、Refactoring用Implementerのように作業目的だけでRoleを分けることを基本とせず、Workflow / Task Type / Stageとの複合scopeで必要Assetを変える。
...
**何を対象に何をするか、およびその作業で何を優先するか**を定義する。

Roleの責務を作業種別ごとに複製せず、feature-development / bug-fix / refactoring等の目的差による変更方針・品質基準・制約はTask TypeやWorkflow Contextへ置き、Roleとの組み合わせで適用する。
...
Implementer Role自体は共通責務を保持し、Issue開発での最小変更やRefactoringでの構造最適化等の作業方針はWorkflow / Task Type固有Assetとして解決する。
...
選択されたWorkflow / Stageに必要な知識。作業全体または特定Stageでのみ成立する変更方針・制約を含む。

### Role Context

役割固有の共通知識・責務。作業目的だけを理由にRole Contextを複製しない。

### Task Type Context

特定のタスク種別に必要な知識・判断基準・変更方針。

Workflow / Task Type / Role等の複合scopeにより、同一Roleへ作業目的に応じたAsset差分を適用できる。
...
複数scopeを組み合わせ、例えば `workflow=issue-development + role=implementer` と `workflow=refactoring + role=implementer` に異なるRule / Skillを適用できる。
...
このAND semanticsにより、Roleを分割せず、`workflow + stage + task-type + role` 等の複合scopeで作業目的固有Assetを限定できる。
...
role = implementer
→ implementer共通Asset + issue-development実装Asset

workflow = refactoring
stage = implementation
role = implementer
→ implementer共通Asset + refactoring実装Asset
```
...
WorkflowはRole自体を作業種別ごとに複製する代わりに、Stage / Task Type / Roleとのrelationやscopeを通じて、そのWorkflowで必要なAsset差分を定義できる。
...
同一Roleに対してWorkflow / Task Type差分で追加されたAssetも、その適用理由とともに確認できることが望ましい。
...
- Roleは共通責務として再利用し、作業目的固有の差分はWorkflow / Stage / Task Type / Role等の複合scopeで限定する
- 作業目的だけを理由にRoleを細分化することを既定手段としない
...
Role中心ビューではRole共通Assetと、Workflow / Task Type等との複合scopeによる条件付きAssetを区別して確認できることが望ましい。
...
作業目的による差分が観測された場合、まずWorkflow / Task Type / Role等の複合scopeで表現可能かを検討し、Role自体の分割は責務そのものが異なる場合に限定する。
...
### Reusable Role + Contextual Specialization

Roleは作業種別ごとに複製せず、共通責務を表す再利用単位とする。同一Roleに必要な作業目的固有のSkill / Rule / Knowledge / Policyは、Workflow / Stage / Task Type / Role等の複合scopeで限定する。
...
- Role管理
- Task Type管理
- Roleを共通責務として再利用し、Workflow / Task Type / Role等の複合scopeで作業目的固有Assetを解決
...
- Workflow / Task Type / Role複合scopeによるContext specialization
- Scope Resolution
- Context Preview
...
44. 同一Roleを複数のWorkflow / Task Typeで再利用し、複合scopeによって作業目的ごとに異なるAsset setを解決できる
...
同じRoleを作業種別ごとに複製するのではなく、Workflow / Task Type等の実行文脈によって、そのRoleへ必要なAssetを切り替える。
